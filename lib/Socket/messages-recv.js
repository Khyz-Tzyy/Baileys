...
const makeMessagesRecvSocket = (config) => {
    ...
    if (ctxInfo?.mentionedJid?.length) {
        const hasLid = ctxInfo.mentionedJid.some(j => j?.endsWith('@lid'));
        if (hasLid) {
            try {
                const metadata = await groupMetadata(msg.key.remoteJid);
                ctxInfo.mentionedJid = ctxInfo.mentionedJid.map(jid => {
                    if (!jid?.endsWith('@lid')) return jid;
                    const lidUser = jid.split('@')[0];
                    const match = metadata.participants.find(p => {
                        return Object.values(p).some(v =>
                            typeof v === 'string' && v.includes(lidUser)
                        );
                    });
                    if (match) {
                        const jidVal = Object.values(match).find(v =>
                            typeof v === 'string' && v.endsWith('@s.whatsapp.net')
                        );
                        if (jidVal) return jidVal;
                    }
                    return jid;
                });
            } catch(e) {}
